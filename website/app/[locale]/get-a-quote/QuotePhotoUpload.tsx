...
export function QuotePhotoUpload({
  ...
  async function addFiles(fileList: FileList | null) {
    ...
    if (remaining <= 0) {
      setMessage(t('photosMax', { count: MAX_PHOTOS }));
      return;
    }
  ...
  return (
    <div>
      <p className="block text-sm font-medium text-foreground">{t('photos')}</p>
      <p className="mt-1 text-xs text-foreground/55">{t('photosHelp')}</p>
      ...
      {!full && (
        ...
        >
          {busy ? t('photosUploading') : t('addPhotos')}
          <input
            id={inputId}
            ref={inputRef}
            type="file"
