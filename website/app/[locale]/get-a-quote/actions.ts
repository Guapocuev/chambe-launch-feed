...
async function pendingRetryCopy(): Promise<string> {
  const t = await getTranslations('Quote');
  const tTime = await getTranslations('ResponseTime');
  return t('pendingRetry', {
    callback: tTime('callbackWindow', {
...
export async function submitJobRequest(
  ...
  if (honeypotFilled(formData)) {
    return { status: 'pending_retry', message: await pendingRetryCopy() };
  }

  if (!fullName || !phone || !address || !description) {
    const t = await getTranslations('Quote');
    return { status: 'error', message: t('needFields') };
  }
  ...
  try {
    ...
    if (res.status === 202) {
      return {
        ...
        message: await pendingRetryCopy(),
      };
    }

    if (res.status === 400) {
      const t = await getTranslations('Quote');
      return { status: 'error', message: body.error ?? t('checkSubmission') };
    }
    ...
    return { status: 'error', message: (await getTranslations('Errors'))('serverError') };
  } catch {
    return {
      status: 'error',
      message: (await getTranslations('Errors'))('unreachable'),
    };
  }
}
